//! Free-variable/mutation analysis for closure literals — vm.md § Closures
//! and anonymous functions, vm.md § Variable capture and boxing. Pure
//! AST-only analysis shared by `nl_codegen::closure` (which decides, per
//! compiled method, which captured names need a boxed slot) and
//! `nl_sema::checker` (which needs the same "captured ∩ mutated" name set to
//! know which `auto`-declared locals need their inferred type propagated
//! back for boxing — see issue #15). Lives here (rather than in either
//! consuming crate) because neither `nl-sema` nor `nl-codegen` depends on the
//! other, but both depend on `nl-syntax`.
//!
//! `referenced_names` walks a closure body's AST and returns every bare name
//! it references (`Expr::Ident`, assignment targets, `++`/`--` operands);
//! callers then check each candidate against the *enclosing* scope's locals
//! to decide which ones are actual captures (a name that isn't an outer
//! local is something else entirely — a class reference, or a name declared
//! inside the closure body itself — and is simply left alone).

use std::collections::HashSet;

use crate::ast::{Block, ClosureBody, Expr, LValue, Stmt, StmtKind};

pub fn referenced_names(body: &ClosureBody) -> HashSet<String> {
...
/// boxed unnecessarily, but boxing is always semantically safe — it only
/// ever adds an indirection, so this over-approximation costs nothing but a
/// little precision.
pub fn boxed_captures_in_block(block: &[Stmt]) -> HashSet<String> {
...
pub fn boxed_captures(body: &ClosureBody) -> HashSet<String> {
    let mut captured = HashSet::new();
    let mut mutated = HashSet::new();
    match body {
...
fn scan_expr(expr: &Expr, captured: &mut HashSet<String>, mutated: &mut HashSet<String>) {
    match expr {
        Expr::Closure { params, body, .. } => {
            ...
            // Recurse into the closure's own body too for *mutations* too
